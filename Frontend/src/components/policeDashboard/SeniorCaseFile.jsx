...
const SeniorCaseFile = () => {
  ...
  // Senior always has access
  const checkAssignment = (c) => true; 
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-[#0f172a] text-slate-200 font-sans pb-10 relative overflow-hidden">
      ...
      <div className="bg-slate-900/80 backdrop-blur-md px-6 py-4 border-b border-slate-700/60 flex justify-between items-center sticky top-0 z-30 shadow-lg relative">
        <div className="flex items-center gap-5">
          {/* ✅ UPDATED: Navigate back to Senior Complaints List */}
          <button onClick={() => navigate("/senior/complaints")} className="text-slate-400 hover:text-white flex items-center gap-2 text-sm font-bold uppercase tracking-wide transition-colors">
            <FaArrowLeft /> Back
          </button>
          <div className="h-8 w-px bg-slate-700 hidden md:block"></div>
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-gradient-to-br from-amber-600 to-yellow-700 text-white rounded-lg flex items-center justify-center text-2xl shadow-lg border border-yellow-500/50"><FaFolderOpen /></div>
            <div>
              <h2 className="text-xl md:text-2xl font-black text-white tracking-widest uppercase flex items-center gap-3">
                Official Case File <span className="bg-red-900/50 text-red-300 text-xs px-2 py-0.5 rounded border border-red-500/30 font-mono tracking-normal">CONFIDENTIAL</span>
              </h2>
              <div className="flex gap-4 mt-1 text-sm items-center font-mono">
                <span className="text-amber-500 px-2 py-0.5 rounded bg-amber-900/10 border border-amber-600/20">ID: {safeIdSlice(complaint._id)}</span>
        ...
        <div className="flex gap-3">
             {isFirFiled && !isClosed && (
                <div className="relative">
                    <button onClick={() => setActiveDropdown(!activeDropdown)} className="flex items-center gap-2 py-2.5 px-5 bg-slate-800 text-blue-400 border border-slate-600 rounded text-sm font-bold uppercase hover:bg-slate-700 transition shadow-sm">
                        <span>Update Status</span><FaChevronDown />
                    </button>
                    {activeDropdown && (
                        <div className="absolute top-full right-0 mt-1 w-56 bg-slate-800 border border-slate-600 rounded shadow-xl z-50 overflow-hidden">
                            {getAvailableStatuses(complaint.status).map((status) => (
                                <button key={status} onClick={() => initiateStatusUpdate(status)} className="w-full text-left px-5 py-4 text-sm font-medium text-white hover:bg-blue-600 border-b border-slate-700 last:border-0 transition-colors">
                                    {status}
                                </button>
                            ))}
                        </div>
      ...
      <div className="bg-slate-900/50 backdrop-blur-md border-b border-slate-800 relative z-20 shadow-md">
        ...
        <div className="bg-[#0b1120]/80 border-t border-slate-800 px-4 md:px-8 py-5 flex flex-col md:flex-row items-start gap-4">
          <div className="bg-amber-500/10 p-3 rounded-full text-amber-500 border border-amber-500/20"><FaExclamationTriangle /></div>
          <div className="flex-1 w-full">
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-amber-500 font-bold uppercase text-sm tracking-widest">Internal Investigation Notes</h3>
              <button onClick={() => setIsEditingNote(!isEditingNote)} className="text-slate-400 hover:text-white text-sm flex items-center gap-1 hover:underline bg-slate-800 px-3 py-1.5 rounded border border-slate-700"><FaEdit /> {isEditingNote ? "Cancel" : "Edit Note"}</button>
            </div>
            {isEditingNote ? (
              <div className="animate-fade-in-up mt-2">
                <textarea value={noteContent} onChange={(e) => setNoteContent(e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded p-4 text-white text-base mb-3 font-mono resize-none focus:border-amber-500 outline-none shadow-inner" rows="4" placeholder="Enter strictly confidential notes..."></textarea>
                <button onClick={saveImportantNote} className="bg-amber-600 hover:bg-amber-500 text-black py-2 px-6 rounded font-bold text-sm uppercase tracking-wide shadow-md"><FaSave className="inline mr-1" /> Save Record</button>
              </div>
            ) : (
              <p className="text-slate-300 italic text-base leading-relaxed font-mono pl-4 border-l-4 border-slate-700 min-h-[24px]">
      ...
      <div className="max-w-[100rem] mx-auto p-6 md:p-10 relative z-10">
        ...
        {(complaint.incidentProof?.length > 0 || complaint.evidence?.length > 0) && (
          <div className="mb-10 bg-slate-800/60 backdrop-blur-sm border border-slate-700 rounded-lg overflow-hidden shadow-lg">
              <div className="bg-slate-900/50 px-8 py-4 border-b border-slate-700 flex justify-between items-center">
                ...
                <span className="text-xs bg-purple-900/30 text-purple-300 px-3 py-1 rounded border border-purple-500/30 font-bold">{complaint.incidentProof?.length + complaint.evidence?.length} FILES</span>
              </div>
                  ...
                  {[...(complaint.incidentProof || []), ...(complaint.evidence || [])].map((url, idx) => {
                    ...
                    return (
                      <div key={idx} className="group relative bg-black rounded-lg border border-slate-700 overflow-hidden shadow-md hover:border-purple-500 transition-all aspect-video">
                        {isVideo ? <video controls className="w-full h-full object-cover" src={url}></video> : isAudio ? <div className="w-full h-full flex flex-col items-center justify-center bg-slate-900 p-2"><FaVolumeUp className="text-4xl text-purple-500 mb-2" /><audio controls src={url} className="w-full h-8 scale-75"></audio></div> : <div className="w-full h-full cursor-pointer relative" onClick={() => setPreviewImage(url)}><img src={url} alt={`Evidence ${idx}`} className="w-full h-full object-cover opacity-80 group-hover:opacity-100" /><div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-black/40"><FaExpand className="text-white text-3xl" /></div></div>}
                        <div className="absolute top-2 right-2 bg-purple-900/90 px-2 py-0.5 rounded text-[10px] text-white uppercase font-bold tracking-wider backdrop-blur-sm border border-purple-500/30">{isVideo ? "Video" : isAudio ? "Audio" : "Img"} #{idx + 1}</div>
                      </div>
                    )
                  })}
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pb-10">
          <div className="bg-slate-800/60 backdrop-blur-sm border border-slate-700 rounded-lg overflow-hidden h-full flex flex-col">
            <div className="bg-slate-900/50 px-8 py-4 border-b border-slate-700"><h3 className="text-cyan-400 text-sm font-extrabold uppercase flex items-center gap-2"><FaFingerprint /> Official Investigation Record</h3></div>
              ...
              {complaint.officialRecord?.recordNumber ? (
                <div className="bg-slate-900/80 rounded-xl border border-emerald-500/30 overflow-hidden shadow-2xl relative">
                    ...
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8 text-sm">
                        <div className="space-y-1"><span className="text-xs text-slate-500 uppercase font-bold block">Filing Date</span><span className="text-white font-medium">{formatDateTime(complaint.officialRecord.recordDate)}</span></div>
                        <div className="space-y-1"><span className="text-xs text-slate-500 uppercase font-bold block">Police Station</span><span className="text-white font-medium uppercase">{complaint.selectedStation}</span></div>
                        <div className="space-y-1"><span className="text-xs text-slate-500 uppercase font-bold block">Investigating Officer</span><span className="text-white font-medium flex items-center gap-2"><FaUser className="text-slate-400 text-xs"/> {complaint.officialRecord.investigatingOfficer}</span></div>
                        <div className="space-y-1"><span className="text-xs text-slate-500 uppercase font-bold block">Rank / Designation</span><span className="text-white font-medium bg-slate-800 px-2 py-0.5 rounded text-xs inline-block border border-slate-700">{complaint.officialRecord.rank}</span></div>
                        <div className="col-span-1 md:col-span-2 pt-2 border-t border-slate-800 mt-2">
                            ...
                            <p className="text-slate-300 text-xs leading-relaxed"><span className="text-slate-400">Date:</span> {complaint.officialRecord.occurrenceDate} | <span className="text-slate-400"> Time:</span> {complaint.officialRecord.occurrenceTime}</p>
                        ...
                        <div className="bg-black/30 p-4 border-t border-slate-800">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {complaint.officialRecord.acts.map((act, i) => (
                                    <span key={i} className="text-xs bg-red-900/20 text-red-300 border border-red-500/30 px-3 py-1 rounded-full font-mono">{act.actName} - Sec {act.section}</span>
                                ))}
                            </div>
                        </div>
                    )}
                    
                    <button onClick={() => handleDownload(complaint)} className="md:hidden w-full bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold py-3 text-center border-t border-emerald-500/30">Download Official Copy</button>
                </div>
              ) : (
                <div className="border-2 border-dashed border-slate-600 p-10 rounded-xl text-center opacity-60 bg-slate-900/30 flex flex-col items-center justify-center h-full min-h-[300px]">
                  <FaLandmark className="text-5xl text-slate-500 mb-4" /><p className="text-slate-400 font-bold text-base uppercase tracking-widest">Pending Official Filing</p><p className="text-slate-600 text-sm mt-2">No FIR/NCR has been generated yet. Investigation officer needs to file the record.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-slate-800/60 backdrop-blur-sm border border-slate-700 rounded-lg overflow-hidden h-full">
            <div className="bg-slate-900/50 px-8 py-4 border-b border-slate-700"><h3 className="text-slate-400 text-sm font-extrabold uppercase flex items-center gap-2"><FaHistory /> Activity Timeline</h3></div>
            <div className="p-8 h-full"><div className="text-slate-200"><TimelineView history={complaint.statusHistory || []} /></div></div>
          </div>
        </div>
      </div>
      ...
      {statusModal.show && createPortal(
        <div className="fixed inset-0 bg-black/80 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 p-8 rounded-lg w-full max-w-lg shadow-2xl relative z-50">
            <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold text-white">Update Case Status</h3><button onClick={() => setStatusModal({ show: false, newStatus: "" })} className="text-slate-400 hover:text-white"><FaTimes /></button></div>
            <label className="text-slate-400 text-xs uppercase font-bold mb-2 block">Official Investigation Remark</label>
            <textarea value={remarkText} onChange={(e) => setRemarkText(e.target.value)} className="w-full bg-black/30 border border-slate-600 rounded p-4 text-white text-base outline-none" rows="5" placeholder="Enter detailed findings..."></textarea>
            <button onClick={submitStatusUpdate} className="w-full mt-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded font-bold">CONFIRM UPDATE</button>
          </div>
        </div>, document.body
      )}

      {previewImage && createPortal(<div className="fixed inset-0 bg-black/95 z-[10000] flex items-center justify-center p-4 backdrop-blur-xl" onClick={() => setPreviewImage(null)}><img src={previewImage} alt="Full View" className="max-w-full max-h-[90vh] rounded border border-slate-600 shadow-2xl" /><button className="absolute top-5 right-5 text-white text-4xl hover:text-red-500"><FaTimes /></button></div>, document.body)}
    </motion.div>
  );
};

export default SeniorCaseFile;