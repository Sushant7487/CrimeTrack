...
const DirectChatWindow = () => {
  ...
  return (
    // ✅ CHANGE 1: `fixed inset-0` added. This locks the viewport on mobile (Whatsapp style).
    <div className="fixed inset-0 flex flex-col bg-slate-900 text-white overflow-hidden">
      ...
      {/* 2. HEADER (Fixed Top - `flex-none` prevents shrinking) */}
      <div className="flex-none bg-slate-800/90 backdrop-blur-md p-3 md:p-4 shadow-lg flex items-center justify-between border-b border-slate-700 z-20">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate(-1)} className="text-gray-300 hover:text-white p-2 rounded-full hover:bg-slate-700 transition">
            <FaArrowLeft />
          </button>
      ...
      {/* 3. MESSAGES AREA (Takes all remaining space - `flex-1`) */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 z-10 custom-scrollbar scroll-smooth">
        {loading ? (
            <div className="flex justify-center pt-20"><div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-blue-500"></div></div>
        ) : messages.length === 0 ? (
      ...
      {/* 4. INPUT AREA (Fixed Bottom - `flex-none` to allow growth upwards) */}
      <div className="flex-none p-3 md:p-4 bg-slate-800 border-t border-slate-700 z-20 w-full">
        
        {/* File Preview Bubble - Pushes input up, not off screen */}
        {file && (
            <div className="mb-2 bg-slate-900 p-2 rounded-lg border border-slate-600 flex items-center justify-between shadow-xl animate-fade-in-up">
                <div className="flex items-center gap-2 overflow-hidden">
                    <span className="bg-slate-700 p-1.5 rounded text-blue-400"><FaPaperclip /></span>
