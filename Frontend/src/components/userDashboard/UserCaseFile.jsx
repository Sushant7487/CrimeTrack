    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-[#0f172a] text-slate-200 font-sans pb-10 relative overflow-hidden">
      ...
      <div className="bg-slate-900/80 backdrop-blur-md px-6 py-4 border-b border-slate-700/60 flex justify-between items-center sticky top-0 z-30 shadow-lg relative">
        <div className="flex items-center gap-5">
          <button onClick={() => navigate(-1)} className="text-slate-400 hover:text-white flex items-center gap-2 text-sm font-bold uppercase tracking-wide transition-colors">
            <FaArrowLeft /> Back
          </button>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 mb-10">
          ...
          <div className="xl:col-span-2 bg-slate-800/60 backdrop-blur-sm border border-slate-700 rounded-lg p-2 shadow-lg h-full flex flex-col relative group">
            ...
            <div className="absolute top-4 right-4 z-10 flex gap-2">
              <button onClick={handleMyLocation} className="bg-blue-600/90 hover:bg-blue-500 text-white p-2.5 rounded text-xs font-bold uppercase flex items-center gap-2 backdrop-blur-sm shadow-lg transition-all" title="Show My Location"><FaLocationArrow /> </button>
              {complaint.incidentLocationAddress?.includes("http") && (
                  <a href={complaint.incidentLocationAddress} target="_blank" rel="noreferrer" className="bg-slate-700/90 hover:bg-slate-600 text-white p-2.5 rounded text-xs font-bold uppercase flex items-center gap-2 backdrop-blur-sm shadow-lg transition-all" title="Open in Google Maps"><FaExternalLinkAlt /></a>
              )}
