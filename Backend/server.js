...
// // Middleware
// // ✅ FIX: Enhanced CORS for Socket.io and Client (Vercel added)
// app.use(cors({ 
//   origin: ["http://localhost:5173", "http://localhost:3000", "https://crimetrack-seven.vercel.app"], 
...
// // Socket Setup
// // ✅ FIX: Enhanced Socket CORS (Vercel added)
// const io = new Server(server, {
//   cors: { 
//     origin: ["http://localhost:5173", "http://localhost:3000", "https://crimetrack-seven.vercel.app"], 
//     methods: ["GET", "POST"],
...
// ✅ Express CORS: Website ke liye jaisa tha waisa hi rakha hai (No issues for web)
app.use(cors({ 
  origin: ["http://localhost:5173", "http://localhost:3000", "https://crimetrack-seven.vercel.app"], 
  methods: ["GET", "POST", "PUT", "DELETE"], 
  credentials: true 
...
// ✅ FIX: Socket CORS '*' kar diya hai taaki Mobile App (bina origin) connect kar sake
const io = new Server(server, {
  cors: { 
    origin: "*", 
    methods: ["GET", "POST"]
...
// ==========================================
// 🔌 SOCKET LOGIC (v1.3 with Admin Protection & Mobile Fixes)
// ==========================================

const userConnections = new Map(); // UserId (String) -> Set<SocketId>
...
io.on('connection', (socket) => {
  ...
  socket.on('register_officer', async (userId) => {
    ...
    // 🔥 FIX: Convert to safe string to prevent ObjectId mismatch
    const safeUserId = String(userId); 

    // ✅ FIX: Ignore Static Admin ID to prevent Database Crash
    if (safeUserId === "ADMIN_ID_STATIC") return;

    if (!userConnections.has(safeUserId)) {
        userConnections.set(safeUserId, new Set());
    }
    userConnections.get(safeUserId).add(socket.id);
    
    socket.join(safeUserId); 

    // Only broadcast "Online" if this is their first connection tab
    if (userConnections.get(safeUserId).size === 1) {
        try {
            await User.findByIdAndUpdate(userId, { isOnline: true }); // DB update with original ID
            io.emit('user_status_change', { userId: safeUserId, status: 'online' });
            console.log(`User ${safeUserId} is Online`);
        } catch (e) {
            console.error("Error updating online status:", e.message);
        }
    }
  ...
  socket.on('send_direct_msg', async (data) => {
    ...
    // 🔥 FIX: Convert to safe string
    const safeRecipientId = String(recipientId); 
    
    const recipientSockets = userConnections.get(safeRecipientId);
    const isRecipientOnline = recipientSockets && recipientSockets.size > 0;

    if (isRecipientOnline) {
        io.to(safeRecipientId).emit('receive_direct_msg', data);
        
        io.to(safeRecipientId).emit('new_notification', {
            type: 'message',
            senderName: data.senderName || "Officer",
            message: data.message
        });
  ...
  // --- 4. TYPING STATUS ---
  socket.on('typing', ({ senderId, recipientId }) => {
      io.to(String(recipientId)).emit('partner_typing', { senderId });
  });

  socket.on('stop_typing', ({ senderId, recipientId }) => {
      io.to(String(recipientId)).emit('partner_stop_typing', { senderId });
  });

  // --- 5. MARK SEEN ---
  socket.on('mark_seen', async (data) => {
    const { senderId, recipientId, messageIds } = data; 
    const safeRecipientId = String(recipientId); // 🔥 FIX: Convert to safe string
    ...
    try {
        ...
        io.to(safeRecipientId).emit('msg_status_update_bulk', { 
            readerId: senderId, 
            status: 'seen'
        });
    } catch (e) {
