...
//Update started for Face Detection 
...
//     // ============================================================
//     // ✅ FACE RECOGNITION & MATCHING LOGIC (ADDED HERE)
//     // ============================================================
//     let suspectFaceDescriptor = [];
//     let matchedCaseId = null;
//     let systemRemark = 'Complaint filed by citizen.';

//     if (suspectIdUrl) {
//         try {
//             console.log("🔍 Processing Suspect Face...");
            
//             // 1. Get Face Descriptor from Service
//             const descriptorFloat32 = await faceService.getFaceDescriptor(suspectIdUrl);

//             if (descriptorFloat32) {
//                 // Convert Float32Array to standard Array for MongoDB storage
//                 suspectFaceDescriptor = Array.from(descriptorFloat32);

//                 // 2. Fetch all previous crimes that have face data
//                 const previousCrimes = await CrimeReport.find({ 
//                     suspectFaceDescriptor: { $exists: true, $not: { $size: 0 } } 
//                 });

//                 if (previousCrimes.length > 0) {
//                     // 3. Prepare Labeled Descriptors for AI Matcher
//                     const labeledDescriptors = previousCrimes.map(crime => {
//                         return new faceapi.LabeledFaceDescriptors(
//                             crime._id.toString(), // Label = Crime ID
//                             [new Float32Array(crime.suspectFaceDescriptor)]
//                         );
//                     });

//                     // 4. Run Matching (Threshold 0.6)
//                     const faceMatcher = new faceapi.FaceMatcher(labeledDescriptors, 0.6);
//                     const bestMatch = faceMatcher.findBestMatch(descriptorFloat32);

//                     if (bestMatch.label !== 'unknown') {
//                         matchedCaseId = bestMatch.label;
//                         console.log("🚨 MATCH FOUND! Linked to Case:", matchedCaseId);
//                         systemRemark += ` | ⚠️ SYSTEM ALERT: Suspect Face Matched with previous Case ID: ${matchedCaseId}`;
//                     }
//                 }
//             }
//         } catch (err) {
//             console.error("❌ Face Recognition Failed:", err);
//             // We continue creating the report even if AI fails
//         }
//     }
//     // ============================================================
...
//       statusHistory: [{ 
//           status: 'Complaint Received', 
//           remark: systemRemark, // Updated remark with match alert
//           updatedBy: 'System', 
//           timestamp: new Date() 
//       }]
//     });
...
//     // ============================================================
//     // ✅ FACE RECOGNITION & SMART HISTORY LOGIC (UPDATED)
//     // ============================================================
//     let suspectFaceDescriptor = [];
//     let matchedCaseId = null;
//     let linkedCases = []; // ✅ NEW: To store history of linked cases
//     let systemRemark = 'Complaint filed by citizen.';
...
//                         // ✅ MAGIC: Fetch the matched case to get ITS history
//                         const matchedCase = await CrimeReport.findById(matchedCaseId);
                        
//                         // A. Add the directly matched case
//                         linkedCases.push(matchedCaseId);

//                         // B. If that case had older history, copy that too! (Chain Reaction)
//                         if (matchedCase && matchedCase.linkedCases && matchedCase.linkedCases.length > 0) {
//                             linkedCases = [...linkedCases, ...matchedCase.linkedCases];
//                         }

//                         // C. Remove duplicates & Ensure IDs are strings
//                         linkedCases = [...new Set(linkedCases.map(id => id.toString()))];

//                         systemRemark += ` | ⚠️ SYSTEM ALERT: Suspect Linked to ${linkedCases.length} Previous Cases!`;
//                     }
//                 }
//             }
//         } catch (err) {
...
//       // ✅ Save New AI Data & History Array
//       suspectFaceDescriptor, 
//       matchedCaseId,
//       linkedCases, // <--- New Field Added
...
//       const isHighRank = req.user.designation === 'DGP' || req.user.designation === 'SP';
//       const isSameStation = req.user.station === report.selectedStation;
//       if (!isSenior && !isHighRank && !isSameStation) { return res.status(43).json({ message: "Jurisdiction Error" }); }
...
// // ==========================================
// // ✅ 8. FACE SEARCH (Search Database by Image)
// // ==========================================
...
//         console.log("🔍 Processing Uploaded Search Image...");

//         // 1. Get Descriptor of uploaded image
//         const queryDescriptor = await faceService.getFaceDescriptor(req.file.path);

//         if (!queryDescriptor) {
//             return res.status(400).json({ message: "No face detected in the uploaded image." });
//         }

//         // 2. Fetch all crimes with face data
//         const allCrimes = await CrimeReport.find({ 
//             suspectFaceDescriptor: { $exists: true, $not: { $size: 0 } } 
//         });

//         const matches = [];

//         // 3. Compare uploaded face with EVERY database face
//         // We calculate Euclidean Distance manually to find ALL matches, not just the "best" one.
//         allCrimes.forEach(crime => {
//             const dbDescriptor = new Float32Array(crime.suspectFaceDescriptor);
//             const queryFloat32 = new Float32Array(queryDescriptor);
            
//             // Calculate distance (lower is better)
//             const distance = faceapi.euclideanDistance(queryFloat32, dbDescriptor);
...
//     updateReportStatus, fileOfficialRecord, triggerSOS, fileFIR, getStats,
//     updateLiveLocation,
//     searchByFace // ✅ Exported correctly
// };
...
// ==========================================
// 4. FETCH REPORTS (✅ UPDATED FOR FACE SEARCH BYPASS)
// ==========================================
const getAllReports = async (req, res) => {
  try {
    const userRole = req.user.role;
    ...
    // ✅ Check if bypassStation query parameter is passed from frontend
    if (req.query.bypassStation === 'true') {
        query = {}; // Allow fetching all reports regardless of station
    } 
    else if (userRole === 'police' || userRole === 'senior') {
        if (userRole === 'senior' || req.user.designation === 'DGP') { 
            query = {}; 
        } 
        else { 
