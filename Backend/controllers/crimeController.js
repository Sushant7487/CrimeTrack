...
//             const userName = `${populatedReport.user.firstName} ${populatedReport.user.lastName}`;
//             const desc = `Official ${recordType} filed. Record No: ${recordNumber}. Officer: ${officerName}`;
//             sendStatusUpdateEmail(populatedReport.user.email, userName, report._id, `${recordType} Filed`, desc).catch(e=>console.error(e));
//         }
...
//           if(remark) { descMsg = remark; report.statusHistory.push({ status: status || report.status, remark: remark, updatedBy: updaterName, timestamp: new Date() }); }
//       }
...
//       if (populatedReport.user && populatedReport.user.email) {
//           const userName = `${populatedReport.user.firstName} ${populatedReport.user.lastName}`;
//           const finalDesc = descMsg || `Status updated to ${statusMsg} by ${updaterName}`;
//           sendStatusUpdateEmail(populatedReport.user.email, userName, report._id, statusMsg, finalDesc).catch(e => console.error(e));
//       }
