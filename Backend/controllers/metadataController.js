...
const Designation = require('../models/Designation');
const LegalSection = require('../models/LegalSection');
const CrimeType = require('../models/CrimeType'); // ✅ ADDED: CrimeType Model
...
const getAllMetadata = async (req, res) => {
  try {
    ...
    const designations = await Designation.find();
    const legalSections = await LegalSection.find();
    const crimeTypes = await CrimeType.find().sort({ name: 1 }); // ✅ ADDED: Fetch Crime Types
    ...
    res.json({
      ...
      crimeTypes: crimeTypes.map(c => c.name) // ✅ ADDED: Send Crime Types array
    });
  } catch (error) {
    console.error("Metadata Error:", error);
    res.status(500).json({ message: "Server Error fetching metadata" });
...
const getCrimeTypes = async (req, res) => {
    const types = await CrimeType.find().sort({ createdAt: -1 });
    res.json(types);
};

const addCrimeType = async (req, res) => {
    const { name, description } = req.body;
    if (!name) return res.status(400).json({ message: "Crime Type Name is required" });
    try {
        const type = await CrimeType.create({ name, description });
        res.status(201).json(type);
    } catch (error) {
        res.status(500).json({ message: "Error adding crime type: " + error.message });
    }
};

const deleteCrimeType = async (req, res) => {
    await CrimeType.findByIdAndDelete(req.params.id);
    res.json({ message: "Crime Type Removed" });
};
...
module.exports = { 
    ...
    getDesignations, addDesignation, deleteDesignation,
    getLegalSections, addLegalSection, deleteLegalSection,
    getCrimeTypes, addCrimeType, deleteCrimeType // ✅ ADDED: Crime Type exports
};
